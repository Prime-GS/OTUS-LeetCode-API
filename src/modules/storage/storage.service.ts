...
import { createWriteStream, unlink, statSync, writeFile, ReadStream } from 'fs'
...
export class StorageService {
  constructor(@Inject(STORAGE_OPTIONS) private readonly options: IStorageOptions) {
    mkdirp.mkdirpSync(this.options.rootPath)
  }

  async uploadFile(file: FileUpload, options: IUploadOptions): Promise<IUploadResponse> {
    ...
    const filePath = await this.storeFile(stream, options.dir, `${name}.${extension}`)
  ...
  private storeFile(stream: ReadStream, dir: string, filename: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      stream.on('error', (error) => writeStream.destroy(error)).pipe(writeStream)
    })
  }

  store(content: string | Buffer, dir: string, filename: string): Promise<string> {
